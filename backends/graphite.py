    ...
    def flushTimers(self, stats, time):
        ...
        for timer_name, timer_vals in stats.timers.items():
            ...
            for pct_threshold in percent_thresholds:
                ...
                msgs.extend((
                    '%(timer_prefix)s.%(timer_name)s.mean_%(pct)d %(mean)d'\
                        ' %(time)d\n' % template_args,
                    '%(timer_prefix)s.%(timer_name)s.upper_%(pct)d %(upper)d'\
                        ' %(time)d\n' % template_args,
                    '%(timer_prefix)s.%(timer_name)s.sum_%(pct)d %(sum)d '\
                        '%(time)d\n' % template_args))
            ...
            msgs.extend((
                '%(timer_prefix)s.%(timer_name)s.mean %(mean)d %(time)d\n'
                    % template_args,
                '%(timer_prefix)s.%(timer_name)s.upper %(upper)d %(time)d\n'
                    % template_args,
                '%(timer_prefix)s.%(timer_name)s.sum %(sum)d %(time)d\n'
                    % template_args))
        print msgs
